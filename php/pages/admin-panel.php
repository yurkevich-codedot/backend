...
$datka_columns = mysqli_query($mysqli, 'SELECT `COLUMN_NAME` FROM `INFORMATION_SCHEMA`.`COLUMNS` WHERE `TABLE_NAME`="' . $_GET['table'].'" ORDER BY ordinal_position');
$data_title = mysqli_query($mysqli, 'SELECT `table_name` FROM `information_schema`.`tables` WHERE `table_name` ="' .$_GET['table'].'"');
$data_tables = mysqli_query($mysqli,'SELECT `table_name` from `information_schema`.`tables` where `table_schema`=`dost`');
            ...
            ?>
              ...
              <span>Сортировать</span>
                <select class="admin-panel__data-kit" name="sort" id="sort">
                  <?
                    $datka_columns = mysqli_fetch_all($datka_columns);
                    foreach($datka_columns as $info)
                    {
                      if($info[0]=='name')
                      {
                        $info[0] = 'Наименование';
